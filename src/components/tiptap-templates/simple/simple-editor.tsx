...
interface SimpleEditorProps {
  /** JSON content từ database (editor.getJSON()). Nếu undefined → dùng content demo mặc định. */
  content?: JSONContent
  /** Callback mỗi khi nội dung thay đổi — nhận editor.getJSON() */
  onChange?: (json: JSONContent) => void
}

export function SimpleEditor({ content: contentProp, onChange }: SimpleEditorProps = {}) {
  ...
  const editor = useEditor({
    ...
    // Dùng contentProp nếu có, ngược lại fallback về content demo
    content: contentProp ?? defaultContent,
    // Gọi onChange mỗi khi editor thay đổi → cập nhật state ở parent
    onUpdate({ editor }) {
      onChange?.(editor.getJSON())
    },
  })

  // Khi contentProp thay đổi từ bên ngoài (vd: load bài edit),
  // sync lại nội dung editor mà không làm mất lịch sử undo.
  useEffect(() => {
    if (!editor || !contentProp) return
    // So sánh tránh loop vô hạn: chỉ setContent khi thực sự khác
    const current = JSON.stringify(editor.getJSON())
    const next = JSON.stringify(contentProp)
    if (current !== next) {
